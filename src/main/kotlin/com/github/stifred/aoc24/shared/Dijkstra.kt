...
class Dijkstra<V, S>(private val costs: MutableList<Step<V, S>>.(V, S) -> Unit) : Search<V, S> {
  override fun bestPathsBetween(start: V, end: V, initialState: S): List<Path<V, S>> {
    ...
    while (toCheck.isNotEmpty()) {
      ...
      for ((next, nextState, nextCost) in buildList { costs(vertex, state) }) {
        if (checkPath.hasSeen(next)) continue
        if (checkPath.totalCost + nextCost > max) continue
...
data class PathBuilder<V, S>(val start: V, val steps: List<Step<V, S>> = listOf()) {
  ...
  fun hasSeen(vertex: V) = steps.any { it.vertex == vertex } || vertex == start
